      ...
      <div class="toolbar">
        ...
        <div class="date-range">
          <div class="date-field">
            <input type="text" id="filterDateFrom" title="Từ ngày" class="date-input" placeholder="dd/mm/yyyy" maxlength="10" inputmode="numeric" autocomplete="off">
            <input type="date" id="filterDateFromPicker" class="date-picker-hidden" tabindex="-1" aria-hidden="true">
          </div>
          <span>đến</span>
          <div class="date-field">
            <input type="text" id="filterDateTo" title="Đến ngày" class="date-input" placeholder="dd/mm/yyyy" maxlength="10" inputmode="numeric" autocomplete="off">
            <input type="date" id="filterDateToPicker" class="date-picker-hidden" tabindex="-1" aria-hidden="true">
          </div>
        </div>
...
function initDashboard(opts = {}) {
  ...
  document.getElementById("filterSearch").addEventListener("input", () => { dashPage = 1; dashTimeMode = "custom"; updateTimeToggle(); applyFilters(); });
  ["filterDateFrom", "filterDateTo"].forEach((id) => {
    ...
    el.addEventListener("input", () => {
      maskDateInput(el);
      picker.value = parseVNDate(el.value);
      dashPage = 1; dashTimeMode = "custom"; updateTimeToggle();
      applyFilters();
    });

    // Bấm vào vùng icon 🗓 (khoảng 26px bên phải ô) sẽ mở lịch của trình duyệt;
    // bấm vùng còn lại vẫn để gõ tay bình thường.
    ...
    picker.addEventListener("change", () => {
      el.value = picker.value ? fmtDateVN(picker.value) : "";
      dashPage = 1; dashTimeMode = "custom"; updateTimeToggle();
      applyFilters();
    });
  });
  ...
  document.getElementById("viewToggle").querySelectorAll("button").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.getElementById("filterDateFrom").value = range.from ? fmtDateVN(range.from) : "";
      document.getElementById("filterDateFromPicker").value = range.from || "";
      document.getElementById("filterDateTo").value = range.to ? fmtDateVN(range.to) : "";
      document.getElementById("filterDateToPicker").value = range.to || "";
      dashPage = 1;
      updateTimeToggle();
      applyFilters();
    });
...
function applyFilters() {
  ...
  const dFrom = parseVNDate(document.getElementById("filterDateFrom").value);
  const dTo   = parseVNDate(document.getElementById("filterDateTo").value);
